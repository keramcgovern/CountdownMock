...
module.exports = function (scene) {
	...
	if (scene.inventory_has_item("Fresh Food")) {
		if (!scene.inventory_has_item("Refrigerator")) {
			...
			if (scene.player.home.location == "dorm") {
				message_queue.push({
					image: "message_eaten_by_rats",
				});
			}
			else {
				message_queue.push({
	...
	if (scene.settings.do_doctor) {
		if (scene.player.money > 0 && scene.player.health < 20) {
	...
	if (doctor_visit || scene.player.hit_by_car) {
	...
	if (scene.player.grade < 60) {
		let m = "Grades too low.\n";

		if (scene.player.job.location != "") {
			scene.player.job.location = "";
			scene.player.job.name = "";
			m += "Lost job.";
		}

		if (scene.player.in_greek_life) {
			scene.player.in_greek_life = false;
			scene.player.paid_dues = false;
			m += "Kicked out of Greek life.\n";
		}

		message_queue.push({
			image: "message_failing",
			message: m,
			text_y: 64,
			font: "chunky"
		});
	...
	if (Phaser.Math.Between(0, 100) < 10) scene.player.account_hold = true;

	//make sure they haven't gone into the negative on any stats
	if (scene.player.money < 0) scene.player.money = 0;
	if (scene.player.happiness < 0) scene.player.happiness = 0;
	if (scene.player.health < 0) scene.player.health = 0;
	if (scene.player.social < 0) scene.player.social = 0;
	if (scene.player.grade < 0) scene.player.grade = 0;
	if (scene.player.energy < 0) scene.player.energy = 0;
