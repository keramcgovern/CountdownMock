...
module.exports = function (scene, max_players, current_player, already_selected = []) {
	...
	for (var i = 0; i < player_data.length; i++) {
		...
		if (!already_selected.includes(i)) {
			var btn_select = scene.bottom_button("btn-select", p_left + player_width / 2, 109, function (scene, data) {
				...
				var btn_play = scene.bottom_button("btn-play", 137, 109, function (scene, data) {
